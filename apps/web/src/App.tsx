...
function App() {
  ...
  useEffect(() => {
    ...
    const availableHandler = electron.update.onUpdateAvailable(
      (data: UpdateEventData) => {
        // 检查是否跳过了此版本（除非是强制检查）
        const skippedVersion = localStorage.getItem("easymd-skipped-version");
  ...
  const [showHistory, setShowHistory] = useState(() => {
    ...
    const saved = localStorage.getItem("easymd-show-history");
    return saved !== "false";
  });
  const [historyWidth, setHistoryWidth] = useState<string>(
    showHistory ? "200px" : "0px",
  );

  useEffect(() => {
    try {
      localStorage.setItem("easymd-show-history", String(showHistory));
    } catch {
      /* 忽略持久化错误 */
    }
  }, [showHistory]);

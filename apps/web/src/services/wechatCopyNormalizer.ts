...
const transformeasymdRootSectionToDiv = (container: HTMLElement): void => {
  const root = container.firstElementChild;
  if (
...
export const stripCopyMetadata = (container: HTMLElement): void => {
  const root = container.firstElementChild;
  if (root instanceof HTMLElement && root.id === "easymd") {
  ...
  container
    .querySelectorAll<HTMLElement>("[data-easymd-counter-generated]")
    .forEach((node) => {
      node.removeAttribute("data-easymd-counter-generated");
    });
};
...
export const normalizeCopyContainer = (container: HTMLElement): void => {
  transformeasymdRootSectionToDiv(container);

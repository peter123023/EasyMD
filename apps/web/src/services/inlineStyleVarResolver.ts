...
export const resolveInlineStyleVariablesForCopy = (html: string): string => {
  ...
  // 临时移除 id="easymd"，阻断预览区暗色 <style> 通过 #easymd 选择器匹配到离屏容器
  const easymdRoot = host.querySelector<HTMLElement>("#easymd");
  if (easymdRoot) easymdRoot.removeAttribute("id");
  ...
  try {
    ...
    if (easymdRoot) easymdRoot.setAttribute("id", "easymd");

...
export const materializeCounterPseudoContent = (
  ...
  host.innerHTML = `<style>${css}</style><section id="easymd">${html}</section>`;
  document.body.appendChild(host);

  try {
    const root = host.querySelector("#easymd");
    if (!root) return html;
    ...
    const processPseudo = (
      ...
      span.setAttribute("data-easymd-counter-generated", pseudo);
